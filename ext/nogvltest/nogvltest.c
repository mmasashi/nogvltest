...
static VALUE rb_mNogvltest_run_once()
{
  int i = 0;
  i += 1;
  return Qnil;
...
static VALUE rb_mNogvltest_run_once_without_gvl()
{
#ifdef CALL_WITHOUT_GVL
  return (VALUE)CALL_WITHOUT_GVL((void *)(rb_blocking_function_t *)rb_mNogvltest_run_once, NULL, NULL, NULL);
#else
  return rb_mNogvltest_run_once();
#endif
}
...
Init_nogvltest(void)
{
  ...
  rb_define_singleton_method(rb_mNogvltest, "run_once", rb_mNogvltest_run_once, 0);
  rb_define_singleton_method(rb_mNogvltest, "run_once_without_gvl", rb_mNogvltest_run_once_without_gvl, 0);
}
